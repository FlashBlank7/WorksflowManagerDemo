...
## 0.6.0 — 2026-08-28（开发中）

看得见「悄悄坏掉的东西」：

- **工作流健康度体检**：平台 `GET /api/v1/health-report` 判 broken（窗口内全败 /
  最近连败≥3）/ stale（有定时却零运行）/ ok；三个入口同一份视图——
  `guanjia doctor` 第 4 项、`guanjia today` 末尾提示、Web 统筹页「需要处理」区块
  （点行直达该工作流详情）
- **运行产物**：时间线展开即列出该次运行落盘的文件，点击经本地壳带令牌下载
- **一致性修复**：失败告警与 `guanjia rerun` 改用应用行当前名——发布版快照存的是
  发布时旧名，改过名的工作流会在告警里出现界面上找不到的名字
- **搭建能力**（平台）：公式引擎补 trim/split/count，len 兼收字符串——
  同需求搭建时间 24.4min → 40s（[A/B 实验](../Lilies/docs/formula-strings-ab-experiment.md)）
- **管家**：生成工作流时给可读短名，不再用需求前 24 字硬截
