...
`guanjia run` / `rerun` 的退出码是对脚本的承诺，不是实现细节：

| 码 | 意思 | 脚本该怎么办 |
|----|------|--------------|
| 0 | 跑成了 | 继续 |
| 1 | 没跑成（失败或被取消） | 当成失败告警 |
| 2 | 命令用法不对（名字找不到、参数不是 `k=v`、缺必填输入） | 改命令，不是改工作流 |
| 3 | 这个状态客户端不认识 | 当成失败处理，并去看一眼远端版本 |
| 4 | 停下等人填（工作流里有人工输入节点） | 别重试，去把那一步填了 |

2 和 1 要分开：一个是你的命令写错了，一个是工作流真的没跑成。
...
的工作流，带上最近一次失败原因；也会单独点出**发布了却一次都没跑过**的那些——
它们没坏，但也没有任何证据说明它好，"都正常"这句话担不起它们。
同一份体检在 `guanjia today`、Web 统筹页、对话里（问「有什么坏了吗」）都看得到。
